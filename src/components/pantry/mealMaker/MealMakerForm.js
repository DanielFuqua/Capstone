...
export default ({
  ...
  const { addMeal, getMeals, meals, updateMeal } = useContext(MealContext);
  const { mealTypes } = useContext(MealTypeContext);
  const { addUserMeal } = useContext(UserMealContext);
  const { addMealFood, mealFoods, updateMealFoods } = useContext(
    MealFoodsContext
  );
  ...
  // ___________EditMode Submission___________
